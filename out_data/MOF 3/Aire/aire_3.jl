cd("/home/gonik/Documents/git/egonik-unlp/QuenchingLFP/out_data/MOF 3/Aire/")
using DataFrames, CSV, LsqFit, Plots, StatsPlots


model(t,p) = p[1] .+ p[2] * exp.(-t*p[3])
filename = "MOF 3 Air 300 700 nm 40 us Delta OD._1.csv"
data = CSV.File(filename) |> DataFrame
df = data[!,["t","460"]]
rename!(df, [:t, :ΔOD])
cutoff = Int(size(df)[1]*.1 ) + 10
dfc = df[cutoff:end, :]
